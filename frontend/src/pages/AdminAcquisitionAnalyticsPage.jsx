...
function Coverage({ data }) {
  ...
  const denominator = data?.denominators || {};
  const backlog = data?.backlogs || {};
  const metrics = [
    ["Master rows", baseline.master_rows, "frozen source-master denominator"],
    ["Existing master IDs", baseline.existing_master_ids, "identity already present"],
    ["Missing-ID rows", baseline.missing_master_ids, "identity review remains separate"],
    ["Unique employer organizations", denominator.unique_employer_organizations, "numeric LinkedIn IDs in baseline"],
    ["Evidence-verified eligible", denominator.evidence_verified_eligible, "explicit manifest/runtime evidence only"],
    ["Organization scan groups", denominator.organization_scan_groups, "explicit runtime grouping"],
    ["Source tasks", denominator.source_tasks, "all durable task rows"],
    ["Runtime explicit employers", denominator.runtime_explicit_employers, "not a substitute for master denominator"],
  ];
  const backlogRows = [
    ["Identity review", backlog.identity_review],
    ["Identity evidence review", backlog.identity_evidence_review],
    ["Alias review", backlog.alias_review],
    ["Negative-result recheck", backlog.negative_result_recheck],
    ["Due retry", backlog.due_retry],
  ].map(([label, value]) => ({ key: label, label, value }));
  const sourceRows = (data?.sources || []).map((row) => ({ ...row, key: row.source_id }));
  return <div className="space-y-5"><Panel title="Company coverage denominator" description="The frozen source-master facts stay separate from runtime task coverage. Unknown means the current manifest or database did not persist an authoritative value."><div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">{metrics.map(([label, value, detail]) => <MetricCard detail={detail} key={label} label={label} value={value} />)}</div><p className="border-t border-outline-variant/10 px-5 py-4 text-sm leading-6 text-on-surface-variant">Baseline input: <span className="font-mono text-xs text-on-surface">{text(baseline.input_sha256)}</span>. Presence is not verification; unresolved or blocked work is not represented as zero jobs.</p></Panel><Panel title="Coverage backlogs" description="These queues are review/retry work, not zero-job company counts."><Table headers={[["label", "Backlog"], ["value", "Items", (row) => count(row.value)]]} rows={backlogRows} /></Panel><Panel title="Company and source coverage" description="One row represents one durable acquisition target and its latest task. Observed, accepted, published and rejected counts remain distinct; failed/deferred work without a result is unknown."><Table headers={[["name", "Company / source"], ["eligibility", "Eligibility", (row) => <span className="inline-flex flex-col gap-1"><Status value={row.eligibility?.operational_state} /><span className="text-xs">Evidence: {row.eligibility?.evidence_verified === null || row.eligibility?.evidence_verified === undefined ? "Unknown" : row.eligibility.evidence_verified ? "Verified" : "Not verified"}</span></span>], ["coverage_state", "Scan state", (row) => <Status value={row.coverage_state} />], ["jobs_observed", "Observed", (row) => count(row.jobs_observed)], ["jobs_accepted", "Accepted", (row) => count(row.jobs_accepted)], ["jobs_published", "Published", (row) => count(row.jobs_published)], ["jobs_rejected", "Rejected", (row) => count(row.jobs_rejected)], ["stop_reason", "Stop reason"], ["freshness", "Freshness", (row) => <span className="inline-flex flex-col gap-1"><Status value={row.freshness?.state} /><span className="text-xs">{row.freshness?.age_seconds === null || row.freshness?.age_seconds === undefined ? "Age unknown" : `${Math.floor(row.freshness.age_seconds / 3600)}h old`}</span></span>], ["next_action", "Next action"]]} rows={sourceRows} empty="No company/source coverage rows are available." /></Panel></div>;
}

function Health({ data }) {
  const queue = data?.queue || {};
  const duration = queue.processing_duration_ms || {};
  const workers = (data?.workers || []).map((row) => ({ ...row, key: row.worker_id }));
  const alerts = (data?.alerts || []).map((row, index) => ({ ...row, key: `${row.code}-${row.worker_id || row.source_id || row.provider || index}` }));
  const resourceText = (row) => {
    const memory = row.resources?.memory_used_bytes === null || row.resources?.memory_used_bytes === undefined ? "Unknown" : count(row.resources.memory_used_bytes);
    const disk = row.resources?.disk_free_bytes === null || row.resources?.disk_free_bytes === undefined ? "Unknown" : count(row.resources.disk_free_bytes);
    return `RAM used ${memory}; disk free ${disk}`;
  };
  return <div className="space-y-5"><Panel title="Queue and service health" description="Queue age, processing, failure/retry counts and database latency are read-only observations. Database latency is a local SELECT measurement, not a Turso/network claim."><div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4"><MetricCard label="Customer queue" value={queue.active_count} detail={queue.oldest_queued_age_seconds === null || queue.oldest_queued_age_seconds === undefined ? "oldest age unknown" : `${Math.floor(queue.oldest_queued_age_seconds / 3600)}h oldest`} /><MetricCard label="Processing average" value={duration.average} detail={`${count(duration.sample_count)} measured samples · ms`} /><MetricCard label="Queue failures / retries" value={`${count(queue.failure_count)} / ${count(queue.retry_count)}`} detail="durable run records" /><MetricCard label="DB latency" value={data?.database?.latency_ms} detail="local read-only SELECT · ms" /></div><div className="border-t border-outline-variant/10 px-5 py-4 text-sm text-on-surface-variant">Stuck tasks: <span className="font-semibold text-on-surface">{count(data?.tasks?.stuck_count)}</span> after {count(data?.tasks?.stuck_after_seconds)} seconds. Retention telemetry: <Status value={data?.limits?.retention?.state} /></div></Panel><Panel title="Workers by role and version" description="Online is computed from heartbeat freshness. A stored running/idle label cannot make an old heartbeat online; missing resource values remain unknown."><Table headers={[["worker_id", "Worker"], ["role", "Role"], ["version", "Version"], ["liveness", "Heartbeat", (row) => <span className="inline-flex flex-col gap-1"><Status value={row.liveness} /><span className="text-xs">{row.heartbeat_age_seconds === null || row.heartbeat_age_seconds === undefined ? "Age unknown" : `${row.heartbeat_age_seconds}s old`}</span></span>], ["stored_status", "Stored status"], ["resources", "Resources", resourceText]]} rows={workers} empty="No durable worker heartbeat is present." /></Panel><Panel title="Provider limits and alerts" description="Provider throttling and budget state are shown only from durable request/budget records. No provider is activated by this screen."><Table headers={[["code", "Alert"], ["severity", "Severity", (row) => <Status value={row.severity} />], ["message", "Detail"]]} rows={alerts} empty="No bounded health alerts are present." /><div className="border-t border-outline-variant/10 px-5 py-4 text-sm text-on-surface-variant">Provider throttling observations: {count((data?.provider_throttling || []).reduce((total, row) => total + Number(row.throttled_requests || 0), 0))}. Spend-limit rows: {count((data?.limits?.providers || []).filter((row) => row.state === "limit_reached").length)}.</div></Panel></div>;
}
...
export default function AdminAcquisitionAnalyticsPage() {
  ...
  const requestParams = new URLSearchParams({ range, timezone });
  if (params.get("start")) requestParams.set("start", params.get("start"));
  if (params.get("end")) requestParams.set("end", params.get("end"));
  const requestPath = `/admin/acquisition/analytics?${requestParams.toString()}`;
