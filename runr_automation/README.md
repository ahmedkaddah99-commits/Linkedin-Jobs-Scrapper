...
- A local provider router that discovers the newest compatible Codex CLI and OpenCode CLI/Desktop shared-auth package, plus an explicit OpenRouter HTTP adapter.
- An OpenRouter implementation loop with four local tools: scoped file read, scoped file write, approved-test execution, and finish.
...
- It does not give OpenRouter a shell, arbitrary filesystem access, Linear credentials, package installation, commit, or deployment capability. OpenRouter is a configurable implementation fallback, not an unrestricted computer agent.
...
Python must report `3.12.7`. Copy `config/runr-automation.example.yaml` to `%LOCALAPPDATA%\RunrAutomation\config.yaml`. The non-secret Linear team ID may be placed in `linear.team_id` or supplied as `RUNR_LINEAR_TEAM_ID`.

For this deployment checkout, create `user_config/.env` with the following names and your real values (never commit this file):

```dotenv
LINEAR_API_TOKEN=<your-Linear-personal-access-token>
RUNR_LINEAR_TEAM_ID=<your-Linear-team-UUID>
OPENROUTER_API_KEY=<your-OpenRouter-key>
```

The controller also accepts `%LOCALAPPDATA%\RunrAutomation\secrets.env`; process environment variables override both files. The key is read in memory, never printed, and never stored in SQLite. `doctor` reports only whether it is configured.
...
5. Providers are tried in the configured `providers.order`. The recommended order is OpenCode subscription, OpenRouter, then Codex. Capacity/transient failure opens a durable circuit and safely hands off to the next provider; Codex is therefore the final fallback by default.
...
runr-auto --repo-root . verify-provider openrouter --run-id openrouter-check-1
```

Each command creates a tiny fixture repository under `%LOCALAPPDATA%\RunrAutomation\verification`, runs all five stages, executes a Python 3.12.7 acceptance test, commits the scoped edit, and stops at approval. Repeating the same run ID proves restart idempotency and does not call the provider again. The OpenRouter check makes real paid API calls and is permitted only when OpenRouter is enabled, keyed, and both budgets are positive.

To create a proposed ticket with the OpenRouter `ticket_creation` model, without mutating Linear:

```powershell
runr-auto --repo-root . draft-ticket --request "Describe the bounded change here" --job-id ticket-draft-001
```

Review that Markdown and create the Linear issue explicitly. The controller's `once` command consumes existing Linear issues; it does not silently create or mutate tickets.
...
- `providers.order`: the complete priority list. Use `opencode_subscription`, `openrouter`, and `codex` in any order. Codex is last in the example.
- `providers.codex.model` and `providers.opencode_subscription.model`: select the model passed to each local CLI.
- `providers.openrouter.models`: choose separate models for `implementation`, `research`, `ticket_creation`, `deduplication`, and `parallelization`. The implementation model is used by the bounded local tool loop; `draft-ticket` uses `ticket_creation`. The current research, deduplication, and parallelization stages are deterministic local Python stages, so their configured model names are reserved until those stages opt into model calls.
- `providers.openrouter.enabled`, `api_key_env`, `base_url`, `max_tool_calls`, `estimated_request_cost_usd`, `max_usd_per_job`, and `max_usd_per_day`: control whether OpenRouter can run and how much it may spend. Both dollar limits must be positive. The estimate is charged to the local ledger when an API response omits usage cost.
