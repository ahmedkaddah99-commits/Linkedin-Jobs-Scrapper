...
class StateStore:
    ...
    def provider_spend(self, *, job_id: str, provider: str, day: str | None = None) -> tuple[float, float]:
        """Return (job spend, UTC-day spend) for one provider."""

        current_day = day or datetime.now(timezone.utc).date().isoformat()
        with self.connect() as connection:
            job = connection.execute(
                "SELECT COALESCE(SUM(amount_usd), 0) FROM provider_spend "
                "WHERE job_id=? AND provider=?",
                (job_id, provider),
            ).fetchone()[0]
            daily = connection.execute(
                "SELECT COALESCE(SUM(amount_usd), 0) FROM provider_spend "
                "WHERE provider=? AND occurred_at LIKE ?",
                (provider, f"{current_day}%"),
            ).fetchone()[0]
        return float(job), float(daily)
    ...
    def record_provider_spend(
        ...
        *,
        ...
        if amount_usd < 0:
            raise ValueError("provider spend cannot be negative")
        with self.connect() as connection:
            connection.execute(
                "INSERT INTO provider_spend(spend_id, job_id, provider, model, amount_usd, occurred_at) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (uuid4().hex, job_id, provider, model, amount_usd, occurred_at or _utc_now()),
            )
