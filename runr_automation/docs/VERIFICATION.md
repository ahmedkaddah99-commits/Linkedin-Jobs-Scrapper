...
- a local HTTP OpenRouter tool-call cycle through a real controller run, scoped commit, tests, and approval wait;
- Ruff static checks;
...
No production deployment, push, merge, live Linear mutation, paid OpenRouter request, worktree deletion, or destructive discard was performed by the repository verification suite. Provider verification uses dedicated repositories under the runtime root. A paid OpenRouter verification is opt-in through `verify-provider openrouter` and requires positive configured budgets.
...
.venv\Scripts\runr-auto.exe --repo-root . verify-provider openrouter --run-id openrouter-check-1
```
