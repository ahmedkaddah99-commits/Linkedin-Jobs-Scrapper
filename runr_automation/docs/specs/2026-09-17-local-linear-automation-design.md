...
It is not a hosted service, webhook receiver, autonomous production deployer, secret store, replacement for Linear workflow states, or guarantee of unlimited Codex/OpenCode capacity. It never silently spends OpenRouter credits.
...
Routing order is Codex CLI, OpenCode subscription, then explicitly enabled and budgeted OpenRouter through OpenCode. Deterministic work stays in Python. Provider commands are discovered by `doctor`, not assumed.
