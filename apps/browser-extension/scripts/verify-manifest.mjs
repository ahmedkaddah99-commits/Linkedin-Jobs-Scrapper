...
assert(
  derivedExtensionId === reservedExtensionId,
  Manifest public key resolves to , expected .,
);
>>>>>>> 59b5a4d (AA-16: Make Chrome installation and permissions store-ready)
...
for (const [size, path] of Object.entries(expectedIcons)) {
  assert(manifest.icons?.[size] === path, Missing manifest icon : );
  assert(manifest.action?.default_icon?.[size] === path, Missing action icon : );
  const bytes = await readFile(join(outputDirectory, path));
  assert(
    bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])),
    Expected a PNG icon: ,
  );
  assert(bytes.readUInt32BE(16) === Number(size), Wrong PNG width for );
  assert(bytes.readUInt32BE(20) === Number(size), Wrong PNG height for );
}

const permissions = new Set(manifest.permissions || []);
for (const permission of ["activeTab", "identity", "scripting", "sidePanel", "storage"]) {
  assert(permissions.has(permission), Missing required permission: );
}
for (const permission of permissions) {
  assert(
    ["activeTab", "identity", "scripting", "sidePanel", "storage"].includes(permission),
    Unexpected production permission: ,
  );
}
...
for (const file of files) {
  ...
  assert(!/\beval\s*\(/.test(source), Remote-code guard: eval found in );
  assert(!/new\s+Function\s*\(/.test(source), Remote-code guard: Function constructor found in );
  assert(!/import\s*\(\s*["']https?:/.test(source), Remote-code guard: URL import found in );
  assert(
    !/\.(?:submit|requestSubmit)\s*\(/.test(source),
    Submission guard: DOM submission API found in ,
  );
  assert(
    !/\b[\w$]*(?:submit|Submit)[\w$]*\s*\(/.test(source),
    Submission guard: submit-like callable capability found in ,
  );
}
...
for (const file of productionBundles) {
  ...
  assert(
    ...
    Production bundle contains AA-01 fixture execution code: ,
  );
  if (file.endsWith("application-form.js") || file.endsWith("background.js")) {
    assert(
      !/\.(?:submit|requestSubmit)\s*\(/.test(source),
      Privileged production bundle contains a DOM submission API: ,
    );
  }
  if (!file.endsWith("background.js")) {
    for (const secretMarker of ["session_token", "sessionToken", "code_verifier", "codeVerifier"]) {
      assert(
        ...
        Extension secret boundary:  escaped the service-worker bundle in ,
      );
    }
  }
}
