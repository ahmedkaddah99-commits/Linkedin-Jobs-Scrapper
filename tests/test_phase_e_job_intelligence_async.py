...
class PhaseEAsyncIntelligenceTests(unittest.TestCase):
    ...
    def test_get_returns_pending_and_worker_publishes_complete_cache(self):
        app = self._backend()
        _seed_catalog(app)
        ...
        with app.repositories.personalized_jobs_store._connect() as connection:
            self.assertEqual(connection.execute("SELECT COUNT(*) AS n FROM job_intelligence_queue WHERE state='queued'").fetchone()["n"], 3)
        while app.process_next_personalized_intelligence() is not None:
            pass
        available = app.get_personalized_job_detail("user-a", "job-a")
        self.assertEqual(available["match_intelligence"]["state"], "available")
