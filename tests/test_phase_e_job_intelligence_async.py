...
from backend.application.personalized_jobs_intelligence import build_intelligence_cache_key, build_match_intelligence
...
class PhaseEAsyncIntelligenceTests(unittest.TestCase):
    ...
    def test_deterministic_fixture_exposes_formula_and_truthful_evidence(self):
        ...
        self.assertEqual(result["v2"]["missing_evidence"], [])

    def test_cache_identity_changes_only_when_relevant_inputs_change(self):
        row = {
        ...
        base = build_intelligence_cache_key(row, intelligence_kind="match", user_id="user-a", profile=profile, evaluator_version="phase_e_v2")
        same = build_intelligence_cache_key(dict(row), intelligence_kind="match", user_id="user-a", profile=dict(profile), evaluator_version="phase_e_v2")
        self.assertEqual(base["cache_id"], same["cache_id"])
        changed_job = build_intelligence_cache_key({**row, "content_hash": "new-hash"}, intelligence_kind="match", user_id="user-a", profile=profile, evaluator_version="phase_e_v2")
        changed_cv = build_intelligence_cache_key({**row}, intelligence_kind="match", user_id="user-a", profile={**profile, "cv_version_id": "cv:v2"}, evaluator_version="phase_e_v2")
        changed_evidence = build_intelligence_cache_key({**row}, intelligence_kind="match", user_id="user-a", profile={**profile, "evidence_version_id": "evidence:v2"}, evaluator_version="phase_e_v2")
        changed_evaluator = build_intelligence_cache_key({**row}, intelligence_kind="match", user_id="user-a", profile=profile, evaluator_version="phase_e_v3")
        self.assertEqual(len({base["cache_id"], changed_job["cache_id"], changed_cv["cache_id"], changed_evidence["cache_id"], changed_evaluator["cache_id"]}), 5)
    ...
    def test_free_can_review_evidence_but_rewrite_requires_pro(self):
        ...
        self.assertTrue(queued["generation"]["cache_id"])
        self.assertEqual(app.process_next_personalized_intelligence()["state"], "available")
        generated = app.improve_personalized_resume("user-a", "job-a", mode="rewrite", plan_id="pro")
        self.assertEqual(generated["state"], "available")
        self.assertEqual(generated["generation"]["document_type"], "tailored_resume")
        self.assertIn("Never claim unsupported", generated["generation"]["guardrail"])
