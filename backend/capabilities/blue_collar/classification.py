...
def build_classification_prompt(
    ...
    if prompt_override.strip():
        ...
        prompt = f"""
You classify grouped blue-collar job role clusters into a fixed category list.
...
- Prefer "other_blue_collar" only when none of the specific categories fit.
...
def call_deepseek_classification(api_key: str, model: str, prompt: str) -> Dict:
    ...
    response = session.post(
        ...
        json={
            "model": model,
            "messages": [
                {"role": "system", "content": "You classify blue-collar job role clusters."},
...
def keyword_classify(cluster: Dict, categories: List[Dict]) -> Tuple[str, str]:
    ...
    best_category = "other_blue_collar"
    best_score = 0
    ...
    if best_score == 0:
        return "other_blue_collar", "Keyword fallback: no strong keyword hit."
            ...
            if parsed and isinstance(parsed.get("assignments"), list):
                for item in parsed["assignments"]:
                    ...
                    if category_id not in valid_category_ids:
                        category_id = "other_blue_collar"
                        reason = f"{reason} Invalid category from AI; fallback to other_blue_collar.".strip()
                    assigned[cluster_id] = {
...
def run_stage3_pipeline(args, *, config: dict | None = None, jobs: List[Dict] | None = None) -> dict[str, Any]:
    ...
    for cluster in clusters:
        ...
        category_id = str(assignment.get("category_id") or "other_blue_collar")
        category = categories_by_id.get(category_id) or categories_by_id.get("other_blue_collar", {})
