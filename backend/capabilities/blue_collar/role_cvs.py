...
ROLE_SUMMARY_BY_CATEGORY = {
    ...
    "other_blue_collar": "Reliable blue-collar worker with practical logistics and operations background, high work ethic, and readiness for physical and shift-based operational tasks.",
}
...
ROLE_SKILLS_BY_CATEGORY = {
    ...
    "other_blue_collar": ["Physical work readiness", "Operational discipline", "Team collaboration", "Punctual and reliable execution"],
}
...
def build_role_cv_text(
    ...
    category_id = str(category.get("id") or "other_blue_collar")
    category_name = str(category.get("name") or "Other Blue-Collar")
    role_summary = ROLE_SUMMARY_BY_CATEGORY.get(category_id, ROLE_SUMMARY_BY_CATEGORY["other_blue_collar"])
    role_skills = ROLE_SKILLS_BY_CATEGORY.get(category_id, ROLE_SKILLS_BY_CATEGORY["other_blue_collar"])
...
def build_stage4_args(config: dict | None = None, overrides: Mapping[str, Any] | None = None) -> SimpleNamespace:
    config = config or load_blue_collar_config()
    payload = {
...
def run_stage4_pipeline(args, *, config: dict | None = None, jobs: List[Dict] | None = None) -> dict[str, Any]:
    ...
    for job in jobs:
        category_id = str(job.get("role_category_id") or "other_blue_collar")
    ...
    baseline_cv_text = load_baseline_cv_text()
