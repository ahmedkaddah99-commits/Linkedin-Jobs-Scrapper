...
def _apply_acquisition_audit_permissions_migration(connection: DatabaseConnection) -> None:
    ...
    Existing administrator-import audit rows are copied into the stream and a
    bridge trigger keeps future legacy writes queryable while old callers keep
    working. The identifier is retained provisionally from the completed
    implementation report; Wave 1 numbering is intentionally not finalized.
    """
