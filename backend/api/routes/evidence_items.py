...
def _handle_post(context: ApiRouteContext) -> bool | None:
    ...
    if segments == ["evidence-items", "migrate"]:
        ...
        if not has_legacy_career_memory(user.metadata):
            context.send_json({"migrated": 0, "message": "No legacy career memory data found."},
                              status=HTTPStatus.OK)
            return True
        evidence = migrate_legacy_facts_to_evidence(
            profile_id=profile_id, user_metadata=user.metadata,
        )
        ...
        store.upsert_many(evidence)
        context.send_json(
            {"migrated": len(evidence), "evidence": [ev.to_dict() for ev in evidence]},
            status=HTTPStatus.CREATED,
        )
        return True
