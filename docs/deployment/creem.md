...
3. Confirm `/billing/subscription` shows `billing_provider=creem`, `plan_id=pro` or `business`, and a `creem_customer_id`.
4. Open the billing portal from the pricing page.
5. Cancel or expire a test subscription and confirm local access returns to `free`.
