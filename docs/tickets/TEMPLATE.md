...
Linear is the system of record for ticket packets, research reports, and attempt evidence. Do not create or commit new `docs/tickets/research/*.md` or `docs/tickets/attempts/*.md` files. If a Markdown file is needed temporarily for a Linear attachment, delete it after Linear confirms the attachment and before committing. Keep durable technical documentation and audit evidence under their owning documentation or `data/audit/` paths.
...
| In Review | `3d37a79a-5846-4d77-86cc-18b660ebce18` |
| Predeployment Integrated | `659fb9cf-d6e4-406e-8062-6043f8b3a7da` |
...
- **Dependencies and parallel-work restrictions:** native blocking relations plus evidence and shared conflict resources. Every blocking relation must declare `dependency-kind` (`implementation`, `release`, or `external`), the required artifact/decision, and `unblocks-when`. An implementation dependency may unblock at an exact `Predeployment Integrated` revision; a release dependency remains blocked until `Ready for Production`.
- **Integration evidence:** when code is on permanent predeployment, record `integration_revision`, `integration_branch`, combined-check result, live-verification state, and whether implementation or release dependents are unblocked.
...
- **Worktree contract:** record the intended branch slug and worktree path pattern; the ticket-start/implementation skill creates the worktree only when explicitly invoked, from the verified permanent predeployment branch `predeployment/render-turso-r2`. Ticket creation never creates it.
...
| `runr-ticket-merge-predeployment` | In Review, or Predeployment Integrated for verification retry | Applicable current label | Ready for Production, or native Done + Predeployment Integrated while live-pending | - | Integration Fix Required | Appropriate blocked status |
...
| `runr-ticket-batch-merge-predeployment` | Selected issues In Review, or Predeployment Integrated for verification retry | Applicable current label | Passed/live -> Ready for Production; passed/live-pending -> native Done + Predeployment Integrated | - | Responsible issues -> Integration Fix Required | Appropriate blocked status |
