...
**T58 update (2026-09-26):** `frontend/src/lib/routePerformance.js` inventories thirteen real-mode route classes and emits privacy-safe User Timing marks from the shared app shell: navigation, connected session, observed useful content/empty/error, and interaction-ready. Dynamic identifiers are excluded from mark names. The `frontend/e2e/route-performance.spec.ts` lab fixture exercises eight top-level routes on desktop and mobile with three cold/warm reload samples, reports p50/p75/p95, and gates each route at <5 seconds through `npm --prefix frontend run check`. It observes a useful card or truthful empty state, not document load. Detail/edit routes are inventoried but do not yet have representative browser fixtures; field LCP/INP/CLS and Render-like timing are not established by this local gate. The historical Phase 2 inventory below predates this update.
