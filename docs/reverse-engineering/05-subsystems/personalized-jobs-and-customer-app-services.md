...
1. **Jobs feed** (`GET /personalized-jobs`): `acquisition_catalog.py` → `services.py:1159 get_personalized_jobs` → `personalized_jobs_service.py:1290 feed`. The flow:
   ...
   - **T54:** the feed response (card and full views) includes a privacy-safe `timings` object with `store_query_ms`, `capabilities_ms` and `total_ms` measured around the store query and the filter-capability projection. `get_published_filter_capabilities` (`sqlite_personalized_jobs.py`) caches the aggregate per publication id — a publication's catalog is immutable, so the result is a pure function of `publication_id` — bounded to 8 cached publications (oldest evicted); a new publication head recomputes. This removes one full-catalog `MAX(CASE WHEN …)` aggregate scan per warm feed request without changing any returned value.
