...
Versioning/restore: `deploy/restore-acquisition-states.sh` validates (`--role all --allow-state-drift --require-table-counts`, so the manifest's restored-volume row counts — LinkedIn `jobs`/`job_company_observations` 188,206, employer `jobs` 2,612 / `companies` 428 — are enforced before activation), canonicalizes via `scripts/canonicalize_producer_states.py` into `/srv/runr/state/versions/<release>`, then atomically switches the `active` symlink (L53–69). `restore-acquisition-states.sh rollback` atomically re-points `active` at the most recent other version dir; no generation is ever deleted, so both pre-restore and restored generations stay recoverable. `deploy/validate_acquisition_runtime.py --evidence-report <path>` captures read-only pre-mutation evidence (release SHA, ExecStart commands, env path names, state overrides, active target, sizes/hashes/schema/counts, rollback target). Checkpoints/backups: `scripts/acquisition_state_backup.py` (SQLite Online-Backup; optional S3/R2 upload; tests `tests/test_rc024_backup_restore.py`).
